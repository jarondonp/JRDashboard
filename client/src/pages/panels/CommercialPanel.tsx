...
const CommercialPanel: React.FC<CommercialPanelProps> = ({ areaId, areaName, color, icon, subtitle }) => {
  ...
  const activeProjects = dashboard.goals.filter((g: any) => g.status === 'en_progreso').length;
  const completedProjects = dashboard.goals.filter((g: any) => g.status === 'completada').length;
  const pendingFollowups = dashboard.tasks.filter((t: any) => t.status === 'pendiente').length;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 p-6">
      <div className="max-w-6xl mx-auto">
                    ...
                    <CardBody>
                      ...
                      <div className="flex gap-2">
                        <span className={`text-xs px-2 py-1 rounded-full font-semibold ${
                          ...
                          {goal.status === 'completada' ? '✓ Cerrado' : goal.status === 'en_progreso' ? 'Activo' : 'Prospecto'}
                        </span>
                        <span className={`text-xs px-2 py-1 rounded-full font-semibold ${
        ...
        <AreaPanelSection
          title="Tareas de Seguimiento y Contacto"
          icon="📞"
          delay={0.3}
        >
          <motion.div
            ...
            <Card>
              <CardBody>
                <div className="space-y-2">
                  {dashboard.tasks.slice(0, 5).map((task: any, idx: number) => (
                    <motion.div
                      key={task.id}
                ...
                <div className="space-y-3">
                  <div className="p-3 bg-blue-50 border-l-4 border-blue-500 rounded">
                    <p className="text-sm text-blue-900">
                      <strong>Conversión:</strong> {dashboard.goals.length > 0 ? ((completedProjects / dashboard.goals.length) * 100).toFixed(0) : 0}% de proyectos completados.
                    </p>
                  </div>
                  <div className="p-3 bg-green-50 border-l-4 border-green-500 rounded">
                    <p className="text-sm text-green-900">
                      <strong>Pipeline Activo:</strong> {activeProjects} proyecto(s) en negociación. Progreso promedio: {dashboard.metrics.avgGoalProgress.toFixed(0)}%.
                    </p>
                  </div>
                  <div className="p-3 bg-purple-50 border-l-4 border-purple-500 rounded">
                    <p className="text-sm text-purple-900">
                      <strong>Acción Recomendada:</strong> Prioriza seguimientos con proyectos de alta prioridad para acelerar cierres.
                    </p>
                  </div>
                </div>
              </CardBody>
